...
class User:
    ...
    def update_log(self, username, password, ip):
        u_p_log = open("UserPassword_log.txt", 'w+')
        u_i_log = open("UserIp_log.txt", 'w+')
    ...
    while True:
        ...
        if name in pass_dict:
            if pass_dict[name] == password:
                ...
                print(getattr(User.message(name)))
                break
            else:
                print("Try again1")
        else:
...
while True:
    selection = input("To login to to a pre-existing account(n).  To exit enter 'e'? (y/n/e)\n")
    if selection == 'y':
        create_user()
    if selection == 'n':
         login(pass_dict)
